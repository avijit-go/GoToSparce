...
ModelYearRoute.get("/list", isAuthenticate, async (req, res) => {
  try {
    ...
    let ModelYearData = await ModelYear.find({ isDelete: { $ne: true } })
      .populate([
        {
          path: "makeId",
          select: "title",
...
ModelYearRoute.get("/detail/:modelyearId", isAuthenticate, async (req, res) => {
  try {
    let ModelYearData = await ModelYear.findOne({
...
ModelYearRoute.patch(
  "/update/:modelyearId",
  isAuthenticate,
  async (req, res) => {
    try {
      const result = await ModelYear.findOneAndUpdate(
        { _id: req.params.modelyearId },
        req.body,
        { new: true }
      );
  ...
  async (req, res) => {
    try {
      ...
      if (ModelYearData.status == true) {
        await ModelYear.findOneAndUpdate(
          { _id: req.params.modelyearId },
          { status: false },
          { new: true }
        );
