...
ModelVariantRoute.get("/list", isAuthenticate, async (req, res) => {
  try {
    ...
    let ModelVariantData = await ModelVariant.find({
      isDelete: { $ne: true },
    })
      .populate([
        {
 ...
 *  @param str modelId
 */
ModelVariantRoute.get(
  "/detail/:modelvariantId",
  isAuthenticate,
  async (req, res) => {
    try {
      let ModelVariantData = await ModelVariant.findOne({
...
ModelVariantRoute.patch(
  "/update/:modelvariantId",
  isAuthenticate,
  async (req, res) => {
    try {
      const result = await ModelVariant.findOneAndUpdate(
        { _id: req.params.modelvariantId },
        req.body,
        { new: true }
      );
  ...
  async (req, res) => {
    try {
      ...
      if (ModelVariantData.status == true) {
        await ModelVariant.findOneAndUpdate(
          { _id: req.params.modelvariantId },
          { status: false },
          { new: true }
        );
...
ModelVariantRoute.delete(
  "/delete/:modelvariantId",
  isAuthenticate,
  async (req, res) => {
    try {
