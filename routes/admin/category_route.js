...
CategoryRoute.post("/create", isAuthenticate, async (req, res) => {
	try {
        const checkCategory = await Category.find({$and:[{title: {$regex: req.body.title, $options: "i"}},{layer:"c"}]});
		if (checkCategory.length){
			for (let index = 0; index < checkCategory.length; index++) {
				if(checkCategory[index].title.toLowerCase() == req.body.title.toLowerCase()){
					return res.status(200).send({error: true, message: "Category exists with this title!"})
				}
			}
		}
        ...
        if(req.body.parentId){
            const checkCategory = await Category.find({$and:[{title: {$regex: req.body.title, $options: "i"}},{layer:"sc"}]});
            if (checkCategory.length){
                for (let index = 0; index < checkCategory.length; index++) {
                    if(checkCategory[index].title.toLowerCase() == req.body.title.toLowerCase()){
                        return res.status(200).send({error: true, message: "SubCategory exists with this title!"})
                    }
                }
            }
            message = {
...
*/
 CategoryRoute.patch("/update/:categoryId",isAuthenticate, async (req, res) => {
	try {
		const result = await Category.findOneAndUpdate({ _id: req.params.categoryId }, req.body, {new: true});
		if (result) {
			message = {
				error: false,
...
CategoryRoute.delete("/delete/:categoryId", async (req, res) => {
    try {
        ...
        return res.status(200).json({message: "Deleted user", status: 200, result });
    } catch (err) {
        message = {
            error: true,
            message: "Operation Failed!",
...
*/
CategoryRoute.get("/changestatus/:categoryId", isAuthenticate, async(req,res) => {
    try{
        let catData = await Category.findOne({_id:req.params.categoryId});

        if(catData.status == true){
            await Category.findOneAndUpdate({_id:req.params.categoryId}, { status: false }, {new: true})
            ...
            await Category.findOneAndUpdate({_id:req.params.categoryId}, { status: true }, {new: true})
