...
BrandRoute.post("/create", isAuthenticate, async (req, res) => {
	try {
        const checkBrand = await Brand.find({title: {$regex: req.body.title, $options: "i"}});
		if (checkBrand.length){
			for (let index = 0; index < checkBrand.length; index++) {
				if(checkBrand[index].title.toLowerCase() == req.body.title.toLowerCase()){
					return res.status(200).send({error: true, message: "An Brand already exists with this title!"})
				}
			}
		}
...
BrandRoute.get("/list",isAuthenticate, async (req, res) => {
    try {
        ...
        let BrandData = await Brand.find(searchVal).sort({_id:-1});
...
BrandRoute.patch("/update/:brandId",isAuthenticate, async (req, res) => {
	try {
		...
		const result = await Brand.findOneAndUpdate({ _id: req.params.brandId }, req.body, {new: true});
		if (result) {
			message = {
				error: false,
...
BrandRoute.patch("/updatePopularBrand/:brandId",isAuthenticate, async (req, res) => {
	try {
		const result = await Brand.findOneAndUpdate({ _id: req.params.brandId },{brandAsPopular:req.body.brandAsPopular}, {new: true});
		if (result) {
			message = {
				error: false,
...
BrandRoute.get("/changestatus/:brandId", isAuthenticate, async(req,res) => {
    try{
        ...
        if(brandData.status == true){
            await Brand.findOneAndUpdate({_id:req.params.brandId}, { status: false }, {new: true})
            ...
            await Brand.findOneAndUpdate({_id:req.params.brandId}, { status: true }, {new: true})
