 ...
 */
 ...
 VariantRoute.get("/detail/:variantid",isAuthenticate,async(req,res)=>{
    try{
        let VariantData = await Variant.findOne({_id:req.params.variantid}).sort({_id:-1});
 ...
 */
 ...
 VariantRoute.patch("/update/:variantid",isAuthenticate, async (req, res) => {
	try {
		const result = await Variant.findOneAndUpdate({ _id: req.params.variantid }, req.body, {new: true});
		if (result) {
			message = {
				error: false,
 ...
 */
 ...
 VariantRoute.get("/changestatus/:variantid", isAuthenticate, async(req,res) => {
    try{
        let variantData = await Variant.findOne({_id:req.params.variantid});

        if(variantData.status == true){
            await Variant.findOneAndUpdate({_id:req.params.variantid}, { status: false }, {new: true})
            ...
            await Variant.findOneAndUpdate({_id:req.params.variantid}, { status: true }, {new: true})
