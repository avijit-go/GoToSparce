...
ProductRoute.post("/create", async (req, res) => {
  try {
    ...
    let checksubcat0Id = await Category.findOne({
      _id: subcat0Id,
      parentId: catId,
    });
    if (!checksubcat0Id) {
    ...
    if (req.body.partNo) {
      let checkOtherRequestedPartNo = await RetailerProtfolio.find({
        partNo: req.body.partNo,
      });
      checkOtherRequestedPartNo = JSON.parse(
        JSON.stringify(checkOtherRequestedPartNo)
      );
...
ProductRoute.get("/:productId", isAuthenticate, async (req, res) => {
  try {
    ...
    ProductData.vehicles = await ProductVechicle.find({
      prodId: prodId,
    }).populate([
      {
        path: "makeId",
...
ProductRoute.get(
  ...
  async (req, res) => {
    try {
      let prodId = req.params.productId;
      let productvehicleData = await ProductVechicle.findOne({
        prodId: prodId,
      });
 ...
 */
ProductRoute.get(
  "/toggle-status/:proVehicleId",
  isAuthenticate,
  async (req, res) => {
    try {
      //let prodId = req.params.productId;
      let productvehicleData = await ProductVechicle.findOne({
        _id: req.params.proVehicleId,
      });

      if (productvehicleData.status == true) {
        await ProductVechicle.findOneAndUpdate(
          { _id: req.params.proVehicleId },
          { status: false },
          { new: true }
        );
