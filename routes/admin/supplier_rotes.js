...
SupplierRoute.get("/list",isAuthenticate, async (req, res) => {
    try {
        ...
        let SupplierData = await Supplier.find(searchVal).sort({_id:-1});
...
SupplierRoute.put("/update/:id",isAuthenticate, async (req, res) => {
	try {
		const result = await Supplier.findOneAndUpdate({ _id: req.params.id }, req.body, {new: true});
		if (result) {
			message = {
				error: false,
...
SupplierRoute.post("/search", isAuthenticate, async (req, res) => {
	try {
		...
		const result = await Supplier.find({
			"$or":[{"name":{"$regex":searchText,$options: 'i' }},{"phone":{"$regex":searchText,$options: 'i' }},{"whatsapp":{"$regex":searchText,$options: 'i'}},{"email":{"$regex":searchText,$options: 'i'}}]})
