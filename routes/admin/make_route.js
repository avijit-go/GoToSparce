 ...
 */
 MakeRoute.post("/create", isAuthenticate, async (req, res) => {
	try {
        const checkMake = await Make.find({title: {$regex: req.body.title, $options: "i"}});
		if (checkMake.length){
			for (let index = 0; index < checkMake.length; index++) {
				if(checkMake[index].title.toLowerCase() == req.body.title.toLowerCase()){
					return res.status(200).send({error: true, message: "An Make already exists with this title!"})
				}
			}
		}
 ...
 */
 ...
 MakeRoute.patch("/update/:makeId",isAuthenticate, async (req, res) => {
	try {
		...
		const result = await Make.findOneAndUpdate({ _id: req.params.makeId }, req.body, {new: true});
		if (result) {
			message = {
				error: false,
...
MakeRoute.patch("/updatePopularMake/:makeId",isAuthenticate, async (req, res) => {
	try {
		const result = await Make.findOneAndUpdate({ _id: req.params.makeId },{makeAsPopular:req.body.makeAsPopular}, {new: true});
		if (result) {
			message = {
				error: false,
...
MakeRoute.get("/changestatus/:makeId", isAuthenticate, async(req,res) => {
    try{
        ...
        if(makeData.status == true){
            await Make.findOneAndUpdate({_id:req.params.makeId}, { status: false }, {new: true})
            ...
            await Make.findOneAndUpdate({_id:req.params.makeId}, { status: true }, {new: true})
