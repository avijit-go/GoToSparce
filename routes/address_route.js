...
AddressRoute.get("/list",isAuthenticate,async(req,res)=>{
    try{
        ...
        let AddressData = await Address.find({$and:[{user_id: userId},{user_type:userType}]});
        message = {
            error: false,
...
AddressRoute.patch("/update/:addressId",isAuthenticate, async(req,res)=>{
    try{
        const result = await Address.findOneAndUpdate({_id:req.params.addressId},req.body,{new:true});
        if(result){
            message = {
                error: false,
 ...
 * * @param str addressId
*/
AddressRoute.get("/set-address/:userId/:addressId",isAuthenticate, async(req,res)=>{
    try{
        // var userId;
        var headers = req.headers;
        var token = headers.authorization.split(' ')[1];
        ...
        if(result?.is_primary == true){
            addressData = await Address.findOneAndUpdate({_id:req.params.addressId},{is_primary:false},{new:true})
        }else{
            addressData = await Address.findOneAndUpdate({_id:req.params.addressId},{is_primary:true},{new:true})
        }
