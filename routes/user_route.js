 ...
 */
 UserRoute.get("/list",isAuthenticate, async (req, res) => {
    try {
            ...
            if (UserData === null) {
                ...
                if (passwordCheck) {
                    if (UserData.status === true) {
                        ...
                        const retailerProfileDetails = await RetailerProfile.findOne({retailerId:UserData._id});
                        ...
                        message = {
                            ...
                            data: [UserData, {retailerProfileDetails:retailerProfileDetails}, {                                
...
UserRoute.post("/search", isAuthenticate, async (req, res) => {
	try {
		...
		const result = await User.find({
			"$or":[{"fname":{"$regex":searchText,$options: 'i' }},{"lname":{"$regex":searchText,$options: 'i' }},{"register_with":{"$regex":searchText,$options: 'i' }},{"mobile":{"$regex":searchText,$options: 'i'}},{"email":{"$regex":searchText,$options: 'i'}}]})
