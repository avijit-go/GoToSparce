...
ReportManagmentRoute.get("/sales/list",isAuthenticate,async(req,res)=>{
    try{
       ...
       // let type = req.query.type;
        ...
        let result = await RetailerOrder.find(findData).populate([
                        {
                            path:"retailerId",
                            select:"fname lname",
                        },
        ...
        ]).select("orderNo orderDate orderPrice items.price items.quantity").sort({_id:-1});
 ...
 */
 ...
 ReportManagmentRoute.get("/sales-report/list",isAuthenticate,async(req,res)=>{
    try{
        let headers = req.headers;
        let token = headers.authorization.split(' ')[1];
        let user = User(token);
       ...
       if(subcatId){
        ...
        let result = await RetailerSales.find(findData).populate([
                        {
                            path:"retailerId",
                            select:"fname lname",
                        },
 ...
 ReportManagmentRoute.get("/stock/list",isAuthenticate,async(req,res)=>{
    try{
       ...
       // let type = req.query.type;
        ...
        let result = await RetailerStock.find(findData).populate([
                        {
                            path:"retailerId",
                            select:"fname lname",
                        },
        ...
        ]).select("entryDate quantity customerorderId supplierorderId reference_type stock_type").sort({_id:-1});
