...
OrderRoute.post("/create", isAuthenticate, async (req, res) => {
  try {
    ...
    for (var i in items) {
      let retailerPro = await RetailerProduct.findOne({
        _id: items[i].proId,
      }).populate([{ path: "proId", select: "title" }]);
      if (!retailerPro)
        return res
          .status(200)
          .send({ error: true, message: "Product is invalid." });

      let retailerProStock = await RetailerStock.find({
        proId: retailerPro._id,
      });
      let totalCount = retailerProStock.reduce(
        (sum, { stock_type, quantity }) => {
      ...
      if (totalCount <= 0)
        return res.status(200).send({
          error: true,
          message: `${retailerPro.proId?.title} is out of stock`,
        });

      let proId = retailerPro.proId._id;
      console.log("proId", proId);
      let proData = await Product.findOne({ _id: proId }).select(
        "title partNo catId subcat0Id subcat1Id mrp images"
      );
      console.log(proData);
...
OrderRoute.get(
  ...
  async (req, res) => {
    try {
      ...
      let orderData = await RetailerOrder.find({
        $and: [
          { retailerId: retailerId },
...
OrderRoute.get("/:id", isAuthenticate, async (req, res) => {
  try {
    ...
    /* +++++++++++++++++++++++++++++++++++++++++++++++++++ */
    let orderData = await RetailerOrder.findOne({
      _id: req.params.id,
    }).populate([
...
OrderRoute.patch("/status-change/:id", isAuthenticate, async (req, res) => {
  try {
    ...
    let orderData = await RetailerOrder.findOneAndUpdate(
      { _id: req.params.id, retailerId: retailerId },
      orderDataBody
    );
