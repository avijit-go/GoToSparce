...
SupplierRoute.put("/:id", isAuthenticate, async(req,res) => {
    try{
        ...
        const result = await RetailerSupplier.findOneAndUpdate({_id:req.params.id},req.body,{new:true});
...
SupplierRoute.get("/list", isAuthenticate, async(req,res) => {
    try{
        ...
        if(searchText){
            result = await RetailerSupplier.find({retailerId:retailerId,$or:[
            ...
            result = await RetailerSupplier.find({retailerId:retailerId}).sort({_id:-1});
        }
