...
ProductRoute.post("/create", isAuthenticate, async(req,res) => {
    try{
        ...
        let checkGTSPro = await RetailerProduct.findOne({retailerId:retailerId,proId:proId});
        if(checkGTSPro){
            message = {
                error: true,
        ...
        let gtsProData = await Product.findOne({_id:proId,status:true}).select('title partNo catId subcat0Id subcat1Id mrp');
...
ProductRoute.put("/:id", isAuthenticate, async(req,res) => {
    try{
        ...
        const result = await RetailerProduct.findOneAndUpdate({_id:req.params.id},req.body,{new:true});
        if(result){
            message = {
                error: false,
        ...
        if (err.name === "ValidationError") {
            ...
            let errors = "Unknown value '"+err.value+"' "+err.kind+" as product "+err.path+" to map";
...
*/
ProductRoute.get("/childcategories/:categoryId",isAuthenticate, async (req, res) => {
    try {
        let categoryData = await Category.findOne({_id:req.params.categoryId,parentId:undefined})

        categoryData = JSON.parse(JSON.stringify(categoryData));

        let subCategories = await Category.find({parentId:req.params.categoryId}).select('title slug parentId image')

        subCategories = JSON.parse(JSON.stringify(subCategories));
        
        for(var i in subCategories){
            let subcatData1 = await Category.find({parentId:subCategories[i]._id}).select('title slug parentId image');
            ...
            for(var j in subcatData1){
                let subcatData2 = await Category.find({parentId:subcatData1[j]._id}).select('title slug parentId');
...
ProductRoute.get("/gts", isAuthenticate, async(req,res) => {
    try{
        ...
        let prodList = await Product.find(searchBy).select('title partNo catId subcat0Id subcat1Id retailerPrice retailerDiscountedPrice ').sort({_id:-1});
...
ProductRoute.get("/list", isAuthenticate, async(req,res) => {
    try{
       ...
       // console.log("proids",proids);
        ...
        if(proids.length>0){
        let product = await RetailerProduct.find({$and:[
            {retailerId: retailerId},
            {proId:{$in:proids}}
        ]}).populate([
                {
    ...
    try{
        ...
        let product = await RetailerProduct.find({$and:[
            ...
            {proId:{$in:proids}}
                ...
                {
                    path:"proId",
                    select:"title catId subcat0Id subcat1Id mrp retailerDiscountedPrice retailerPricePercentage customerDiscountedPrice customerPricePercentage partNo pro_no images brandId",
                    populate:([
                        {
                            path:"catId",
                            select:"title"
