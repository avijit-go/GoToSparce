...
RetailerCouponRoute.get("/list", isAuthenticate, async (req, res) => {
  try {
    const CouponData = await RetailerCoupon.find({
      isDelete: { $ne: true },
    }).sort({ _id: -1 });
    message = {
      error: false,
...
RetailerCouponRoute.patch(
  "/update/:CouponId",
  isAuthenticate,
  async (req, res) => {
    try {
      const result = await RetailerCoupon.findOneAndUpdate(
...
RetailerCouponRoute.delete(
  "/delete/:CouponId",
  isAuthenticate,
  async (req, res) => {
    try {
      const result = await RetailerCoupon.deleteOne({
        _id: req.params.CouponId,
      });
