...
CustomerRoute.get("/list", isAuthenticate, async (req, res) => {
  try {
    ...
    if (searchText) {
      customers = await RetailerCustomer.find({
      ...
      customers = await RetailerCustomer.find({ retailerId: retailerId }).sort({
        _id: -1,
      });
    }

    let totalCustomers = await RetailerCustomer.find({
      retailerId: retailerId,
    }).count();
...
CustomerRoute.get("/:id", isAuthenticate, async (req, res) => {
  try {
    ...
    /* +++++++++++++++++++++++++++++++++++++++++++++++++++++ */
    var retailerId = user.data._id;
    let customer = await RetailerCustomer.findOne({
      _id: req.params.id,
    }).populate([
      {
        path: "makeId",
...
CustomerRoute.put("/update/:id", isAuthenticate, async (req, res) => {
  try {
    ...
    /* +++++++++++++++++++++++++++++++++++++++++++++++++++++ */
    const result = await RetailerCustomer.findOneAndUpdate(
      { _id: req.params.id },
      req.body,
      { new: true }
    );
...
CustomerRoute.post("/create-address", isAuthenticate, async (req, res) => {
  try {
    ...
    let customerAddresses = await RetailerCustomerAddress.find({
      customerId: req.body.customerId,
    });
    if (customerAddresses.length == 1) {
      await RetailerCustomerAddress.findOneAndUpdate(
        { _id: resultAddress._id },
        {
          is_primary: true,
        },
        { new: true }
