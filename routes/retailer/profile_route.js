...
ProfileRoute.post("/save", isAuthenticate, async(req,res) => {
    try{
        ...
        let existProfile = await RetailerProfile.findOne({retailerId:retailerId});

        if(existProfile){
            let resProfile = await RetailerProfile.findOneAndUpdate({ retailerId: retailerId }, req.body, {new: true})
...
ProfileRoute.get("/view", isAuthenticate, async(req,res) => {
    try{
        ...
        let profile = await RetailerProfile.findOne({retailerId:retailerId});
        message = {
            error: false,
            message: "My profile",
            data: profile
...
ProfileRoute.post("/add-shipping-address", isAuthenticate, async(req,res) => {
    try{
        ...
        let existSameAddress = await RetailerShippingAddress.findOne({retailerId:retailerId, $and: [{address: req.body.address},{lat: req.body.lat},{long: req.body.long}] });
...
*/
ProfileRoute.put("/edit-shipping-address/:id", isAuthenticate, async(req,res) => {
    try{
        let headers = req.headers;
        let token = headers.authorization.split(' ')[1];
        let user = User(token);
        ...
        let result = await RetailerShippingAddress.findOneAndUpdate({_id:req.params.id}, req.body, {new: true})
...
ProfileRoute.get("/list-shipping-address", isAuthenticate, async(req,res) => {
    try{
        ...
        let result = await RetailerShippingAddress.find({retailerId:retailerId}).sort({_id:-1});
...
*/
ProfileRoute.get("/detail-shipping-address/:shippingaddressId", isAuthenticate, async(req,res) => {
    try{
        let headers = req.headers;
        let token = headers.authorization.split(' ')[1];
        let user = User(token);
        ...
        let result = await RetailerShippingAddress.findOne({_id:req.params.shippingaddressId}).sort({_id:-1});
