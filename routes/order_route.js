...
OrderRoute.patch("/payment-status", isAuthenticate, async(req, res) => {
    try {
        ...
        const orderData = await Order.findOneAndUpdate({_id: req.body.orderId}, paymentbody, {new: true})
        if(!orderData) return res.status(200).send({error: true, message: "order not found"})
        return res.status(200).send({error: false, message: "order payment status changed", orderData})
...
OrderRoute.get("/:orderId", isAuthenticate, async(req,res) => {
    try{
        ...
        let orderData = await Order.findOne({_id:orderId}).populate([
            { 
                path: 'order_details.proId', select: '' ,
                populate: [
...
OrderRoute.delete("/delete/:id", async(req, res) => {
    ...
    return res.status(200).json({message: "Deleted order", status: 200, wishlist: updatedOrder})
})
