 ...
 */
 ...
 ReviewRoute.get("/list",isAuthenticate, async(req,res)=>{
    try{
        ...
        let ReviewData = await Review.find({$and: [{ user_id: userId }, {user_type:userType}]}).populate([
            {
                path:"prodId",
 ...
 */
 ...
 ReviewRoute.delete("/delete/:id", async (req, res) => {
    try {
        ...
        return res.status(200).json({message: "Deleted review", status: 200, wishlist: updatedReview})
    } catch (err) {
        message = {
            error: true,
            message: "Operation Failed!",
