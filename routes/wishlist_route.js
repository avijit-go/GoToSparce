 ...
 */
 ...
 WishlistRoute.post("/create",isAuthenticate, async(req,res)=>{
    try{ 
        ...
        const existWishlistProduct = await Wishlist.findOne({ $and: [{ user_id: userId }, { proId: req.body.proId }] });
...
WishlistRoute.delete("/delete/:wishlistId", async (req, res) => {
    try {
        ...
        return res.status(200).json({message: "Deleted", status: 200, wishlist: updatedishlist})
    } catch (err) {
        message = {
            error: true,
            message: "Operation Failed!",
